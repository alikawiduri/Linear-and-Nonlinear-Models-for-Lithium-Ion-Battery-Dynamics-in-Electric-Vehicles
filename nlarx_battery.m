...
orders_nlarx = [8 8 8 8 1 1 1];

NL = idTreePartition;
sys_nlarx = nlarx(data_est,orders_nlarx,NL);

%% TRAINING FIT
[~,fit_train] = compare(data_est,sys_nlarx);

%% VALIDATION FIT

[~,fit_val] = compare(data_val,sys_nlarx);

%% RESULT

fprintf('\n');
fprintf('============================\n');
fprintf('NLARX RESULTS\n');
fprintf('============================\n');
...
figure
compare(data_val,sys_nlarx)
grid on
title('NLARX Validation')
...
title('Measured vs Predicted Voltage using NLARX')

grid on

exportgraphics(gcf,...
'NLARX_Prediction.png',...
'Resolution',300);

%% Prediction Data

[y_nlarx,fit_nlarx_val] = compare(data_val,sys_nlarx);

%% Save
save('battery_nlarx_results.mat',...
    'sys_nlarx',...
    'y_nlarx',...
    'fit_nlarx_val',...
    'data_val');